...
(function attachSnorkelShared(global) {
  ...
  function buildForecastUrls(region, options = {}) {
    ...
    const marineUrl =
      ...
      `&hourly=wave_height,wave_direction,swell_wave_height,swell_wave_period,swell_wave_direction,wind_wave_height,wind_wave_period,sea_surface_temperature,ocean_current_velocity,ocean_current_direction,sea_level_height_msl` +
  ...
  async function fetchForecast(region, options = {}) {
    ...
    const hourly = times.map((time, index) => ({
      ...
      waveDir: marineData.hourly.wave_direction?.[index] ?? 0,
